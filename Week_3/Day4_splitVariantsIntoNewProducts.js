...
function splitVariantsIntoNewProducts(datoEntity, variantLimit = 2) {
    const id = null;
    const title = datoEntity.attributes.title;
    const sku = datoEntity.attributes.sku;
    const expectedVarients = [];
    let expected = {
        id,
        title,
        sku,
    ...
    const objectLength = datoEntity.attributes.variants.length;
    let counter = 0
    let contObject = {};
    for(let i = 0; i < objectLength; i++) {
        if(counter < variantLimit) {
            let varientInfo = datoEntity.attributes.variants[i];
            let varientName = varientInfo.attributes.name;
            let varientPrice = varientInfo.attributes.price;
            contObject.name = varientName;
            contObject.price = varientPrice;
            expectedVarients.push(contObject);
            
            contObject = {};
            counter++
        }
        else if(counter === variantLimit ) {
            counter = 0;
        }
    }
    return expected;
